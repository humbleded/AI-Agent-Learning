...
- 每周留 **1 个工作日做机动**（默认周三，或你固定加班的那天），不排硬任务，专门吸收顺延。
...
- 周三（机动 / 缓冲）：读 HA 第 1 章《初识智能体》建立大局观；加班则只读 Hub「What is an agent」。
...
- 周三（机动）：读 HA 第 9 章《上下文工程》上半（S-03 资料）。
...
- 周三（机动）：**算法启动**——随想录「数组」专题 + 2 道；加班只看题解。
