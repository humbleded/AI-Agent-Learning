...
| Agent 原理与控制循环 | LEARNING | A4-01～A4-05 已通过；2026-08-06 模拟面试中 ReAct、Plan-and-Solve、上下文清洗与停止边界稳定；A4-Gate Problem Contract 已完成设计静态核验 | 完成 A4-Gate 可运行循环、14 条 eval、结构化日志、工具失败/超时恢复和 HITL 真实轨迹；补幂等与端到端事实一致性 | A4-Gate、G8-00、旗舰二 |
...
| 安全与权限 | LEARNING | 文件沙箱、工具白名单、参数校验已有练习；A4-Gate 合同已定义相对路径拒绝边界、不可恢复删除前确认和 `confirmed` 执行门槛 | 把设计落实为真实危险分支、审计轨迹和攻击测试；再补威胁模型、注入/SSRF/越权/外泄/poisoning/MCP auth | A4-Gate、S-05、D7/G8/M9/FINAL |
...
| 算法与系统设计 | LEARNING | W4 已正式独立 PASS 1 道数组/哈希题；存在重复元素通过 8 条显式测试与 9 条隐藏边界，能说明平均 `O(n)` / 额外 `O(n)` | 清完 W4 其余 3 道债务，持续累计 50–70 道高质量题、错题复测，并补 RAG/对话服务系统设计 | algorithm-progress、J11-08 |
