...
- 收口规则（2026-09-05 修订）：本债务保留 `RETRY / INCOMPLETE`，用独立岗位审计 Session 补齐或重审候选，形成恰好 10 条 `HARD_ELIGIBLE` 后才关闭。它不再阻塞满足技术依赖的 `R6-01`；W12 必须复查未关闭原因并保留各自快照。正式匹配率、J11-Gate 和投递前就绪仍要求完整审计，不能因继续学习而抹掉债务。
