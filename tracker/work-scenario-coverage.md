...
| --- | --- | --- | --- | --- | --- | --- | --- |
| WS-01 | 需求澄清与 API/事件契约 | 模糊需求、冲突验收条件、接口兼容、变更影响 | A4-Gate | A4/BE5/R6/E10/J11 | DIAGNOSED_AND_FIXED | `daily/2026-08-05.md`～`2026-08-13.md`：合同、C1～C3d 与 C4a-1 落地；最终三字段 / 来源子集硬门、SDK 响应外壳、首次模型精确 `finish_reason == "tool_calls"`、五步调用事件合同及 topic/path 正常链均已闭合 | A4-Gate C4a-2（一次工具重试、失败状态 / action recovery 与安全停止复合轨迹） |
| WS-02 | 日志、指标与运行轨迹排错 | 错误堆栈、request ID、错误率/p95、根因与表象 | A4-Gate | BE5/G8/E10/J11/FINAL | DIAGNOSED_AND_FIXED | `daily/2026-08-12.md`～`2026-08-13.md`：实现单请求 `request_id`、共享 step 与固定七字段日志；五个真实调用点均按 `reserve → 计时 → 调用 → 互斥日志` 接线。topic/path 严格 step 1～5、双请求隔离和第 7 次调用在 clock/create/log 前拒绝均通过；时间戳误作 step、异常对象入日志、硬门顺序及协议字面量错误均经注入定位并回归 | A4-Gate C4a-2（retryable 工具两次真实尝试分别记账，并闭合失败状态映射）；E10 再补完整 tracing / 指标 |
...
| WS-05 | 模型/provider 韧性与成本 | 429、Retry-After、退避+jitter、配额耗尽、fallback、context 超限 | T3-04/S-01 | BE5/E10/FINAL | RECOVERED_UNDER_FAULT | `daily/2026-08-04.md`：真实 Reflection/Refinement 故障恢复 12/12；`daily/2026-08-08.md`～`2026-08-21.md`：工具、Candidate 与 Reflection 的响应 / Provider 故障、受控恢复和安全停止逐层闭合；`daily/2026-08-22.md`：Refinement 的 Provider、响应、JSON 与 candidate-contract 跨失败域共享一次 recovery，49/49 主审、10/10 独立复核与真实正常链通过，无第三次请求 / step 7 | A4-Gate（补 HITL 与 14 条整关 eval）；BE5-Gate（补 Retry-After、退避与 fallback） |
| WS-06 | 身份、授权与多租户 | 猜 ID、跨用户读写、ACL filter、principal 丢失、日志越权 | BE5-05 | BE5/R6/M9/FINAL | NOT_VERIFIED |  | BE5-Gate |
| WS-07 | 工具与 Agent 安全 | 坏参数、路径逃逸、SSRF、间接注入、secret/PII 外泄、过度授权 | T3-03/T3-Gate | A4/R6/M9/FINAL | RECOVERED_UNDER_FAULT | A4 C1～C3d（`daily/2026-08-07.md`～`2026-08-11.md`）：工具白名单、请求原值绑定和严格成功 Observation 均已闭合；`daily/2026-08-22.md`：Refinement 越界来源 candidate 被丢弃，坏 candidate / URL / 原异常不进入 recovery prompt，新 candidate 重过 validator；非法白名单在 SDK 前拒绝，失败公开来源从真实工具结果重派生并防御复制 | A4-Gate（补 HITL 允许 / 拒绝与危险动作 eval）；R6/M9 安全复测 |
