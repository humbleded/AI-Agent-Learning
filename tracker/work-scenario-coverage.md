...
| --- | --- | --- | --- | --- | --- | --- | --- |
| WS-01 | 需求澄清与 API/事件契约 | 模糊需求、冲突验收条件、接口兼容、变更影响 | A4-Gate | A4/BE5/R6/E10/J11 | DIAGNOSED_AND_FIXED | `daily/2026-08-05.md`～`2026-08-10.md`：合同与 C1～C3d-C3a 落地；除 Tool Calling 消息合同外，已闭合 Observation 派生 allowlist、Reflection / Refinement prompt 与不可信候选的精确三字段 / 来源子集硬门 | A4-Gate（补两次 Reflection / Refinement 模型调用与 JSON 解析集成，并完成失败 + 七字段日志 + 重试 / 安全停止复合轨迹） |
...
| WS-03 | 测试、CI 与回归 | 单测通过但集成失败、flaky、eval 回归、发布门禁 | P0-07/L1-Gate | BE5/E10/J11/FINAL | DIAGNOSED_AND_FIXED | `daily/2026-08-08.md`～`2026-08-10.md`：C2b 补出 1xx 假绿；C3c 忠实 SDK fake 补出协议 / 外壳缺口；C3d 分层 sentinel 与非法类型超集又定位 `and/or`、`all([])` 及类型证明前 `len/.strip/set`，最终独立 C3d `160/160` | A4-Gate（补 C3d 双路径端到端与 14 条 Agent 全量 eval）；BE5-Gate |
...
| WS-07 | 工具与 Agent 安全 | 坏参数、路径逃逸、SSRF、间接注入、secret/PII 外泄、过度授权 | T3-03/T3-Gate | A4/R6/M9/FINAL | DIAGNOSED_AND_FIXED | A4 C1～C3d-C3a（`daily/2026-08-07.md`～`2026-08-10.md`）：工具白名单、请求原值绑定和严格成功 Observation 均已闭合；允许来源只从本轮 path / `results[*].url` 派生，最终候选的虚构或混合越界来源由客户端 fail-closed | A4-Gate（补 C3d 模型 / JSON 集成、HITL 允许 / 拒绝、受控 `needs_manual` 与安全停止） |
