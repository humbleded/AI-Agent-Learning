...
| --- | --- | --- | --- | --- | --- | --- | --- |
| WS-01 | 需求澄清与 API/事件契约 | 模糊需求、冲突验收条件、接口兼容、变更影响 | A4-Gate | A4/BE5/R6/E10/J11 | DIAGNOSED_AND_FIXED | `daily/2026-08-05.md`～`2026-08-24.md`：从模糊 topic/path 需求冻结到 FINAL Contract，并落实请求、Tool Action/Observation、五步账本、失败/恢复、exact success 与 HITL；唯一 V5 RUN-5 完整固定集 14/14，五问闭合 | BE5/R6 继续把合同迁移到版本化 HTTP/API 与领域证据边界；E10 管理兼容与评估变更 |
| WS-02 | 日志、指标与运行轨迹排错 | 错误堆栈、request ID、错误率/p95、根因与表象 | A4-Gate | BE5/G8/E10/J11/FINAL | DIAGNOSED_AND_FIXED | `daily/2026-08-12.md`～`2026-08-24.md`：单请求 `request_id`、共享 step、固定七字段日志与调用前 reserve 已覆盖正常 5/5、F01 4/4、F02 5/5、F03 6/6、工具 retry 和 HITL timeout/replay；错误只记稳定类型名，普通异常保持 identity 上抛 | E10 补 tracing/指标/回归关联；G8 补 checkpoint/replay 的跨进程轨迹 |
| WS-03 | 测试、CI 与回归 | 单测通过但集成失败、flaky、eval 回归、发布门禁 | P0-07/L1-Gate | BE5/E10/J11/FINAL | DIAGNOSED_AND_FIXED | `daily/2026-08-08.md`～`2026-08-24.md`：V4 RUN-4 RETRY 不覆盖，按透明问题修复、holdout 替换、V5 指纹冻结后唯一 RUN-5 为 14/14；正式收口离线 70/70、独立 85/85 + 55/55，源码/eval 指纹无漂移 | BE5 建维护型 pytest/lint/type-check/CI；E10 再建版本化 evaluator 与回归门禁 |
