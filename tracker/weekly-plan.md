...
最后校准：2026-07-22。W4 已收尾：T3-01~T3-Gate 全部 PASS；A4-01~A4-03 已 PASS，下一主任务为 A4-04 Plan-and-Solve；算法 0/4 保持 W4 `DEBT`，只在独立算法时段结转。

## 你的时间与排法

- 可持续基线：工作日每天 2.5–3 小时（约 12.5–15h）+ 周末 1–2 个各 3 小时的连续块（约 3–6h）= **约 15.5–21 小时 / 周**。每天 3 小时是全天总预算；27 小时只可作为用户主动选择的短期冲刺，不用于承诺课程进度。
- 默认每个自然日只有一个 ≤3 小时块。只有用户明确说「今天不上班，状态好，按休息日双块安排」或同义口令，才启用第一块 ≤3 小时 + 充分休息 + 第二块 2–3 小时的短期冲刺，全天最多 6 小时；第二块不追加概念题量，未执行也不形成补偿欠账。
- 时间线：约 **18 个内容块到综合项目上线**，约 **20 个内容块含求职冲刺**。
- **节奏说明（重要）**：下表 `W1–W20` 是内容块编号，不是日历周。按 15.5–21h/周、零基础带读、工程化验收和回收周估算，现实更可能需要 **28–36 个日历周**；把 W 当“第几块内容”，不当“第几周必须到”。
...
- 排法原则（按你的时间形状）：
  - **工作日最多 3h**：概念、文档、源码、即时考察、定向正式练习和 notes 共用预算。算法只有日计划明确排了独立时段时才替换部分主块。
...
- 诚实提醒：
  - 15.5–21h/周仍是高投入学习；某周顶不住就顺延，节奏比赶进度重要。
...
| W6 | 阶段4 | J11-01、S-01 设计、首轮双轨 JD 审计（大厂标杆 + ≥7 个硬可投社招）；算法独立时段 | A4-Gate 若上块未完成则先收尾；否则推进旗舰二 v1 | 旗舰二 v1 + 可投岗位基线 |
...
- 这版按每周 15.5–21h 的可持续基线估算，且允许一个内容块跨多个日历周。现实里会有波动，**把它当节奏参考，不是 deadline**。
