...
本清单用于每天记录学习、提交代码、回答检查题，并由 Codex 判定是否通过。以后所有代码、笔记、源码阅读、完成情况都放在这个文件夹里。
...
说明：Codex 检查时会根据代码/产物位置实际运行代码并验证结果。只有当代码不是默认入口、需要特殊参数、需要先启动服务或配置环境变量时，才填写“运行提示”。

`Codex 当天进度总结`、`Codex 总进度总结`、`Codex 对问题/不确定点的解释`、`Codex 问答点评与补充`、`Codex 超前内容提示` 由 Codex 检查后填写。遇到的问题如果属于后续阶段内容，Codex 不要求当天掌握，只标注应该学到哪个任务或阶段再深入。
