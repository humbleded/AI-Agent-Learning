...
实际工作问题以 `tracker/work-scenario-coverage.md` 为事实源。`T3-Gate` 先完成一个受控复合危险输入；从 `A4-Gate` 起，编码、Gate、项目练习至少包含一个绑定真实代码/日志/trace/失败测试的工作场景。普通小编码任务围绕当前真实改动验证至少一个相关故障；只有两类以上已学问题存在实际联系时才组合，不为凑类别重考旧知识或提前引入后续能力。从 `BE5` 起的 Gate/项目验收仍至少包含一个同时叠加 2–4 类问题的复合事故，各 Gate 明定的更高数量与恢复要求全部保留。一个事故可以覆盖多个类别，但必须分别记录“止损/隔离 → 复现 → 根因定位 → 修复 → 回归/恢复 → 取舍”，纯口述不能冒充可执行证据。
...
- 在实际动手 Session 创建 `code/stage6/engineering_docs_rag/` 前，先按项目设计分工说明用户决策、助手整理与最终审核的职责；固定目标使用者、一个主要查找/排障场景、公开或已授权资料范围、最小成功例和权限边界。既有证据能唯一确定的内容由助手整理，用户只作新增且影响行为的判断并最终审核。助手把这些决策整理为本项目已有规划的 `problem-contract.md` 最小版本，不要求用户逐节抄写，也不提前设计完整检索/API/部署；后续任务随新增证据更新，R6-Gate 再收口。
- 然后用 LangChain Loader、Text Splitter 与 Document 对象/metadata 字段建立可运行的导入切片；本任务只实现 Loader、Splitter、Document/chunk 生命周期与轻量确定性测试。
...
- 本任务正式检查时同时核验 `S-02` 的独立通过标准并分别写回状态；复用同一覆盖图、作答、代码与运行证据，不另出第二套题或重复实现。S-02 已达标就在本次收口；若仍有缺口，只补对应缺口并如实保留状态，不能只称作“已隐含覆盖”，也不能无故拖到 R6-Gate。后续 Gate 验新增集成及受改动影响的行为。
...
- 核查并更新从 R6-01 开始的 `problem-contract.md`：以已确认的开发/运维用户、主要场景和资料边界为基础，补齐查找工程事实和排障依据的原流程、输入/输出、权限、引用必须满足的规则、验收、人工接管，以及至少 1 个可测业务/操作指标；不在实现完成后才第一次决定用户问题。
...
- 先用现有单 Agent 版本建立质量、延迟、成本和失败类型 baseline；把“最小学习候选实验”与“产品默认采用”分开，未测得净收益前继续保留单 Agent 为默认。
- 本任务只负责提出可证伪的协作实验问题，优先针对可定位的瓶颈；未发现瓶颈时如实以“协作是否只增加复杂度和成本”为待验证问题，不伪造需求。冻结最小角色职责、带类型约束的交接字段、共享/私有 state、停止/预算/升级、指标和回退条件。
- `S-04` 紧接着在同一 LangGraph“故障诊断与变更评审 Agent”中实现最小候选实验，不另起 supervisor 玩具项目；两项分别验收。候选实验不等于默认采用，产品是否保留多 Agent 由同集对照决定；不能因最终保留单 Agent 就省略 S-04 的协作机制实作证据。
...
- 单 Agent baseline 的表现、已有问题或“尚未发现瓶颈”的证据可定位；能说明确定性节点、单 Agent 与受控多 Agent 各自的采用条件，不按热词选架构。
...
- 至少记录一项模式的适用边界与保留、限制或回退依据，优先复用当前或既有实验的失败、负收益或成本代价。若本次实验确有净收益，按真实结果说明代价，并给出明确标注为风险推演的适用边界；不为满足题面而强造“反而更差”的结果，也不把推演计为已执行故障证据。
...
- 在单 Agent baseline 通过后，复用 `S-04` 已完成的最小受控多 Agent 候选实验与同集对照：只拆分能被独立验证的诊断/风险审查职责，使用带类型约束的交接格式、共享/私有 state、最大 handoff 次数、预算、停止和人工升级。Gate 只补新增集成或受改动影响的验证，不再重建一套协作实验；是否默认采用按预设质量、成本与延迟门槛决定，无净收益则保留单 Agent 并记录回退，完整实作证据仍须保留。
...
- 工具复用现有工程文档 RAG 的授权资料读取，或故障诊断旗舰的只读 runbook/诊断能力；本文件只做 MCP server 适配，不重建第三个业务项目或复制 Agent 循环。
...
- 把 MCP client 接到选定旗舰已有的工具边界，沿用它的模型调用、权限、日志与错误处理；本文件保留可运行的连接/调用入口，除本任务所需的 MCP server 外，不新增另一套产品 HTTP 服务、业务状态或部署工程。
...
- 保留 `code/stage9/m9_gate_file_summary_agent.py` 作为集成验收入口，调用 M9-02/03 与选定旗舰现有能力；不因历史文件名重新实现文件摘要 Agent。
- 现有 RAG 或故障诊断旗舰通过 MCP 读取授权目录中的工程资料/runbook，并用于其问答或诊断主链；至少演示本地 STDIO 与受保护 HTTP 中一种真实主链，另一种保留可运行最小样例。
...
- STDIO 或受保护 HTTP 中选定的一种旗舰真实主链，以及另一种可运行最小样例，都可重复执行；工具发现、正常调用、超时/断连/重连、权限拒绝、审计和长结果裁剪有自动测试。两种选择按相同标准判定，真实模型验证不得由全 mock 替代。
...
- 将上述实质性改动用于维护一个此前未独立修改过的现有模块：从 issue 或明确标注的模拟反馈出发，定位入口和一条相关调用链，读取现有测试，复现问题或用失败测试表达新的验收条件，再做最小修改、相关回归与回退说明。可用已经引入的他人代码、尚未独立读懂的助手代码或 H5 的等价证据，如实注明来源；用户亲自解释根因、影响范围和要保护的旧行为。与 J11-06 的反馈迭代共用同一次改动，不另加一个 PR 或新项目。
- 在已有维护改动中完成一次真实依赖的版本候选评估，可与上一项共用：读取官方发布/迁移说明，说明升级理由和受影响接口，保留旧版本依赖与启动方式；在隔离环境锁定候选版本，运行已有相关测试及受影响的真实集成验证，再验证恢复旧版本仍可运行。可以采用或拒绝候选，不以“升到最新”作为目标；复用原 PR/变更说明，不另建升级课程或报告系统。
...
- 能用上述已有改动展示陌生模块定位、最小维护、旧行为回归，以及依赖候选采用/拒绝和恢复旧版本的真实证据；明确区分助手操作与用户独立判断。
...
- 另一个旗舰进入维护状态：其代码、依赖、共享模块或评估配置变化时，按影响范围复跑相关验证；作品交付前再做可复现检查。主产品的无关修改不触发另一旗舰的全量真实模型评估。
...
挂载：阶段 6，默认在 R6-02 正式检查时收口，R6-Gate 只核查新增集成与改动。与 R6 共用真实产物、覆盖图、作答和运行证据，但保留独立任务状态；分别对照本节标准写回，达标即收口、仅补缺口，不重复出题或重做实现，也不能永久停在 TODO 或被“隐含覆盖”。
...
- 先保留同一业务、同一数据集和同一指标的单 Agent baseline；按 D7-02 冻结的问题完成一次最小学习候选实验，再判断是否值得默认采用。优先验证上下文隔离、职责专门化或并行调查的可定位瓶颈；没有瓶颈时如实检验协作是否只增加复杂度，不倒推需求或预设必须有收益。
- 在故障诊断项目中用可切换配置实现一条最小受控协作候选链，例如 coordinator 只负责分派和停止、diagnostic worker 只读取证、risk reviewer 独立检查证据与变更风险；每个 handoff 使用可验证 schema，不依赖自然语言猜测隐含状态。实作与同集对照是学习要求，进入产品默认路径是另一个由证据决定的选择。
...
为什么：部分高工程标准岗位明确要求 multi-agent orchestration，但当前可投样本频率可能低于主线阈值；最小候选实验用于掌握安全协作与判断方法，产品采用依据是实测的上下文隔离、独立复核或可控协作价值，而不是框架热度。
