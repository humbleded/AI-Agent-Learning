...
| A4-Gate | 最小 Agent 闯关 | DOING | 2026-08-13 | code/stage4/problem-contract.md + code/stage4/a4_gate_research_summary_agent.py + code/stage4/eval_cases.json | Problem Contract、C1/C2、C3a～C3d 与 C4a-1 已闭合。C4a-1 已实现单请求 `run_context`、固定七字段调用日志、五个真实模型/工具调用点的共享 `request_id` 与严格 step 1～5、`max_steps=6` 调用前硬门、请求隔离，以及首次模型 `finish_reason == "tool_calls"` 守门；topic/path 无重试集成、双请求隔离与第 7 次调用零副作用拒绝均通过。父 Gate 仍缺 C4a-2 一次工具重试与失败状态/恢复映射、HITL allow/deny、14 条 eval（文件尚未创建）及正式 Gate 复核。 |
