...
| A4-Gate | 最小 Agent 闯关 | DOING | 2026-08-21 | code/stage4/problem-contract.md + code/stage4/a4_gate_research_summary_agent.py + code/stage4/eval_cases.json | Problem Contract、C1/C2、C3a～C3d、C4a-1、C4a-2、C4b-1、C4b-2a、C4b-2b-1、C4b-2b-2a 与 C4b-2b-2b 已完成教学 / 实现子单元：已闭合共享 step 与七字段日志、工具一次重试及失败映射、一次 Action correction、Candidate / Reflection 的协议内容恢复与 Provider/API 一次受控恢复、SDK retry / timeout 所有权、两套恢复计数、完整下游链预算、安全公开映射、故障注入、真实正常链及用户最终行为审核。父 Gate 仍缺 Refinement Provider/API、响应外壳 / JSON 与最终 validator 失败恢复，HITL allow/deny，14 条 eval（文件尚未创建），五个 Gate 必答问题，Problem Contract 总体定稿及正式 Gate 复核。 |
