...
| A4-Gate | 最小 Agent 闯关 | DOING | 2026-08-13 | code/stage4/problem-contract.md + code/stage4/a4_gate_research_summary_agent.py + code/stage4/eval_cases.json | Problem Contract、C1/C2、C3a～C3d、C4a-1、C4a-2、C4b-1、C4b-2a 与 C4b-2b-1 已完成教学 / 实现子单元：已闭合共享 step 与七字段日志、工具一次重试及失败映射、一次 Action correction、Candidate 协议 / 内容失败的一次受控恢复，以及 Reflection 协议 / 内容失败的一次受控恢复；两类恢复均受完整下游链预算约束并已完成故障注入、真实正常链和用户最终行为审核。父 Gate 仍缺 Candidate / Reflection Provider/API 异常分类与恢复、Refinement / 最终 validator 失败恢复、HITL allow/deny、14 条 eval（文件尚未创建）、五个 Gate 必答问题、Problem Contract 总体定稿及正式 Gate 复核。 |
