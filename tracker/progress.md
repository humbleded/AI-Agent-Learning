...
| A4-Gate | 最小 Agent 闯关 | DOING | 2026-08-22 | code/stage4/problem-contract.md + code/stage4/a4_gate_research_summary_agent.py + code/stage4/eval_cases.json | Problem Contract、C1/C2、C3a～C3d、C4a-1、C4a-2、C4b-1、C4b-2a、C4b-2b-1、C4b-2b-2a、C4b-2b-2b 已完成教学 / 实现子单元，`C4b-2c` 已正式 `PASS`：共享 step 与七字段日志、工具一次重试及失败映射、一次 Action correction、Candidate / Reflection / Refinement 的协议内容恢复与 Provider/API 受控恢复、SDK retry / timeout 所有权、完整链预算、Refinement 跨失败域共享唯一 recovery、JSON / validator 错误所有权及安全公开映射均已闭合；本次正式复验 49/49、独立 10/10、真实 DeepSeek `sample.txt` 正常链通过。父 Gate 仍缺 HITL allow/deny、14 条 eval（文件尚未创建）、五个 Gate 必答问题、Problem Contract 总体定稿及正式 Gate 复核。 |
