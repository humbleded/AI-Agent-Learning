...
| A4-Gate | 最小 Agent 闯关 | DOING | 2026-08-10 | code/stage4/problem-contract.md + code/stage4/a4_gate_research_summary_agent.py + code/stage4/eval_cases.json | Problem Contract、C1/C2、C3a～C3c，以及 C3d-C1 来源白名单、C2a Reflection prompt、C2b Refinement JSON-only prompt、C3a 最终三字段 / 来源子集硬门已闭合；尚缺 Reflection → Refinement 两次模型调用、响应外壳、`json.loads` → validator、topic/path 端到端，以及日志、统一 `max_steps` / 重试安全停止、HITL、14 条 eval 与正式 Gate 复核。 |
