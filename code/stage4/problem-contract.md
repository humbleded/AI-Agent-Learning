...
- `summary`：类型为 `str`，填写基于实际资料形成的摘要正文，不填写工具动作或虚构结论。
- `sources`：类型为 `list[str]`，每个元素必须是生成摘要时实际使用过的 URL 或沙箱相对路径，不得填写工具名、搜索词或虚构来源。
...
所有失败结果仍必须保留三个顶层字段。`summary` 必须是非空字符串并说明具体失败原因；`sources` 只能包含本次实际取得的来源，没有真实来源时必须为 `[]`，不得编造。若失败前已经取得部分真实来源，可以原样保留这些来源，但不能据此把结果标记为成功。
...
- 摘要初稿或 Reflection 候选结果未通过第 4 节合同或来源真实性校验时，只能依据本轮真实证据修订；达到 `max_steps` 仍不能形成合法结果时，停止并返回 `needs_manual`，不得把未通过校验的候选结果直接返回。
...
- 所有拒绝与失败结果均保持第 4 节固定三字段结构，说明具体原因，并且只保留已经实际取得的来源。
